...
module.exports = tobz = async (tobz, message) => {
    ...
    const truth = [
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            found === true 
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Chika`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 12){
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh Chika!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 1X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nPeringatan! 2x lagi berkata kasar, Chika akan kick kamu dari grup!`, id)
                        }
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Chika!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
        ...
        function GenerateSerialNumber(mask){
            ...
            return serialNumber;
                ...
                if(body === '${prefix}mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Chika!', id)
               ...
               if (chats.match("chika") || chats.match("Chika") || chats.match("CHIKA") || chats.match("bot") || chats.match("Bot") || chats.match("BOT")) {
                ...
                if (body === '${prefix}unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Chika!', id)
        ...
        case prefix+'ttpdef':
                ...
                try
                {
                    const string = body.toLowerCase().includes('${prefix}ttpdef') ? body.slice(5) : body.slice(5)
                    if(args)
                    {
        ...
        case prefix+'ttp':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}ttp2 [ Teks ]*, contoh * ${prefix}ttp2 Chika*`, id)
        ...
        case prefix+'motor':
                ...
                try {
                    ...
                    tobz.sendFileFromUrl(from, resp.data.result.image, 'gsm.jpg', kbbuaww, id)
                } catch (err) {
                    console.error(err.message)
                    await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, tidak ditemukan')
                    tobz.sendText(ownerNumber, 'Motor Error : ' + err)
                }
                break
        case prefix+`spek`:
                ...
                try {
                    ...
                    console.error(err.message)
                    await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, tidak ditemukan')
                    tobz.sendText(ownerNumber, 'Spek Error : ' + err)
                }
                break
        case prefix+`mobil`:
                if (isLimit(serial)) return tobz.reply(dari, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                ...
                try {
                    ...
                    if (resp.data.error) return tobz.reply(from, resp.data.error, id)
                    const kbbuaww = `➸ *Title* : ${resp.data.result.title}\n➸ *Harga* : ${resp.data.result.harga}\n➸ *Spesifikasi* : ${resp.data.result.spesifikasi}\n➸ *Kelebihan* : ${resp.data.result.kelebihan}\n➸ *Kekurangan* : ${resp.data.result.kekurangan}`
                    from.sendFileFromUrl(from, resp.data.result.image, 'gsm.jpg', kbbuaww, id)
                } catch (err) {
                    console.error(err.message)
                    await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, tidak ditemukan')
                    tobz.sendText(ownerNumber, 'Mobil Error : ' + err)
                }
                break
       case prefix+'dare':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
       ...
       case prefix+'truth':
        ...
        case prefix+'magernulis1': // BY MFARELS
                if (args.length === 4) return await tobz.reply(from, 'Kirim Perintah * ${prefix}magernulis1 --[Nama]--[Kelas]--[Teks]*\n\n*Contoh :*\n${prefix}magernulis1 --MFarelS--7B--Subscribe MFarelS CH', id) // https://github.com/MFarelS/RajinNulis-BOT
        ...
        case prefix+'blackpink':
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}blackpink [ Teks ]*, contoh * ${prefix}blackpink Chika*`, id)
        ...
        case prefix+'pornhub':
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}pornhub [ |Teks1|Teks2 ]*, contoh * ${prefix}pornhub |Tobz|Dev Chika*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah * ${prefix}pornhub [ |Teks1|Teks2 ]*, contoh * ${prefix}pornhub |Tobz|Dev Chika*`, id)
            }
            break
...
${desc}`)
        ...
        case prefix+'simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Chika!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
        ...
        case prefix+'otakudesu':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}otakudesu [query]*\nContoh : * ${prefix}otakudesu darling in the franxx*', id)
...
🌐️ *URL*: ${url}`
        ...
        case prefix+'getvideo':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}getvideo* [IdDownload], untuk contoh silahkan kirim perintah * ${prefix}readme*', id)
            try {    
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}getmusik [IdDownload]*, untuk contoh silahkan kirim perintah * ${prefix}readme*')
                if (args[1] <= 25) return tobz.reply(from, `*Apabila ditag hanya cantumkan nomer urutan bukan ID Download!*\nContoh : * ${prefix}getmusik 1*`,)
                tobz.reply(from, mess.wait, id)
        ...
        case prefix+'youtubesearch':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}youtubesearch [query]*, Contoh :  ${prefix}youtubesearch alan walker alone')
        ...
        case prefix+'brainlysearch':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}brainlysearch [query]*, Contoh : * ${prefix}brainlysearch Siapa itu Elaina?*')
        ...
        case prefix+'nhentai':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah * ${prefix}nhentai* [ Query ]')
        ...
        case prefix+'getnhentai':
           ...
           try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}getnhentai [ Id Download ]*, untuk contoh silahkan kirim perintah * ${prefix}readme*`, id)
                    if (!Number(args[1])) return tobz.reply(from, `*Apabila ditag hanya cantumkan nomer urutan bukan ID Download!*\nContoh : * ${prefix}getnhentai 1*`, id)
                    const dataDownmp3 = quotedMsg.type == 'chat' ? quotedMsg.body : quotedMsg.type == 'image' ? quotedMsg.caption : ''
                    const pilur = dataDownmp3.split('(#)')
                    console.log(pilur[args[1]])
                    tobz.reply(from, mess.wait, id)
                    ...
                    if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}getnhentai [ Id Download ]*, untuk contoh silahkan kirim perintah * ${prefix}readme*`)
        ...
        case prefix+'xvideos':
            ...
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah * ${prefix}xvideos* [ Lagu ]`)
            ...
            try {
                ...
                let berhitung = 1
                let xixixi = `*「 XVIDEOS 」*\n\n*Hasil Pencarian : ${querVID}*\n\n─────────────────\n\nKetik  ${prefix}getxvideos [angka] untuk mengambil ID, Contoh :  ${prefix}getxvideos 2\n`
                for (let i = 0; i < result.length; i++) {
        ...
        case prefix+'xxx':
            ...
            try {
                ...
                let berhitung = 1
                let xixixi = `*「 XVIDEOS 」*\n\n*Hasil Pencarian : ${querXXX}*\n\n─────────────────\n\nKetik  ${prefix}getxxx [angka] untuk mengambil ID, Contoh :  ${prefix}getxxx 2\n`
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'nh':
            ...
            if (args.length === 2) {
                ...
                tobz.reply(from, '[ WRONG ] Kirim perintah * ${prefix}nhentai [kode]* untuk contoh kirim perintah * ${prefix}readme*')
            }
            break*/
        case prefix+'brainly':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
       ...
       /* case prefix+'jadwaltv':
        ...
        case prefix+`cersex`:
            ...
            tobz.reply(from, `silahkan command dengan pilihan * ${prefix}️cersex1* atau * ${prefix}cersex2*`, id)
            break
       ...
       case prefix+'️indohot':
            ...
            break
        case prefix+'️cersex2':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}️limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN Chika
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Chika', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Chika', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Chika', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Chika', id)
            await tobz.sendText(from,'Chika DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan Chika`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan Chika`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Chika atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Chika!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Chika!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Chika!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Chika!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Chika!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Chika!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Chika!`, id)
        ...
        case prefix+'ping':
            ...
            await tobz.reply(from, `Penggunaan RAM: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB\n_CPU:_ ${os.cpus()[0].model}\n\n_Status :\n- _${loadedMsg} Loaded Messages\n- ${groups.length} Group Chats\n- ${chatIds.length - groups.length} Personal Chats\n- ${chatIds.length} Total Chats\n\n*Status HP Bot*\n${(`\n*Battery :* ${battery}% ${isCharging ? 'Lagi Di Cas...' : 'Ga Di Cas!'}\n${Object.keys(me.phone).map(key => `${key} : ${me.phone[key]}`).join('\n')}`.slice(1, -1))}\n\n\n*Speed:* Kenceng Telkomsel Wuzz!`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Chika', id)
