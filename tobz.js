...
INGAT JANGAN JUAL SCRIPT NEKO KEPADA ORANG LAIN!
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗] Kirim gambar dengan caption *❤️sticker* atau tag gambar yang sudah dikirim',
                Ti: '[❗] Replay sticker dengan caption *❤️toimg* atau tag sticker yang sudah dikirim',
        ...
        const isUrl = new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#️?&/=]*)/gi)
        ...
        // FUNCTION
                ...
                if (isGroupMsg && isDetectorBadword && !isGroupAdmins && !isAdmin && !isOwner){
                    if (chats.match("anjing") || chats.match("gblk") || chats.match("tolol") || chats.match("kntl")) {
                        if (!isGroupAdmins) {
                            ...
                            }).catch(() => tobz.sendText(from, `Untung NEKO Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                        } else {
                ...
                if(body === '❤️mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin NEKO!', id)
                        ...
                        tobz.reply(from, 'Bot telah di mute pada chat ini! ❤️unmute untuk unmute!', id)
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                        ...
                        muted.push(chatId)
                        fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
                        reply(from, 'Bot telah di mute pada chat ini! ❤️unmute untuk unmute!', id)
                    }
                }
                if(body === '❤️unmute' && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin NEKO!', id)
                        if(isMsgLimit(serial)){
                            return
                ...
                if (body === '❤️unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner NEKO!', id)
        ...
        case '❤️banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner NEKO!', id)
        ...
        case '❤️ttp':
                ...
                try
                {
                    const string = body.toLowerCase().includes('❤️ttp') ? body.slice(5) : body.slice(5)
                    if(args)
                    {
        ...
        case '❤️ttg':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            try {
                if (quotedMsgObj == null) {
                    if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️ttg [ Teks ]*, contoh *❤️ttg aku bukan boneka*`, id)
        ...
        case '❤️toimg':
            ...
            break
        case '❤️stickergif': // INSTALL FFMPEG, IF YOU WANT THIS COMMAND WORK!
        case '❤️stikergif': // TUTORIAL IN README, PLEASE READ!
        case '❤️sgif': // MRHRTZ
            ...
            if (isMedia && type === 'video' || mimetype === 'image/gif') {
                ...
                tobz.reply(from, `Kesalahan ⚠️ Hanya bisa video/gif apabila file media berbentuk gambar ketik ❤️stickergif`, id)
            } 
            break
        case '❤️stickerlightning':
        case '❤️slightning':
             tobz.reply(from, `[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 min!`, id)
            if (isMedia && type === 'image') {
                const mediaData = await decryptMedia(message, uaOverride)
                ...
                await tobz.reply(from, `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan ❤️stickerlightning`, id)
            }
            break
        case '❤️stickerfire':
        case '❤️sfire':
            ...
            if (isMedia && type === 'image') {
                ...
                await tobz.reply(from, `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan ❤️stickerfire`, id)
            }
            break
        case '❤️groupinfo' :
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', message.id)
...
${desc}`)
        ...
        case '❤️tts':
            ...
            await limitAdd(serial)
            try {
                if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️tts [ Bahasa ] [ Teks ]*, contoh *❤️tts id halo semua*')
                var dataBhs = args[1]      
        ...
        case '❤️kapankah':
            ...
            if (!when) tobz.reply(from, '⚠️ Format salah! Ketik *❤️menu* untuk penggunaan.')
        ...
        case '❤️rate':
            ...
            if (!rating) tobz.reply(from, '⚠️ Format salah! Ketik *❤️menu* untuk penggunaan.')
        ...
        case '❤️apakah':
            ...
            if (!nanya) tobz.reply(from, '⚠️ Format salah! Ketik *❤️menu* untuk penggunaan.')
         ...
         case '❤️bisakah':
            ...
            if (!bsk) tobz.reply(from, '⚠️ Format salah! Ketik *❤️menu* untuk penggunaan.')
        ...
        case '❤️nsfw':
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                tobz.reply(from, 'NSFW berhasil di aktifkan di group ini! kirim perintah *❤️nsfwMenu* untuk mengetahui menu', id)
        ...
        case '❤️simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin NEKO!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
                fs.writeFileSync('./lib/database/Simsimi.json', JSON.stringify(simi_))
                tobz.reply(from, 'Simsimi berhasil di aktifkan di group ini! Kirim perintah *❤️ [teks]*\nContoh : *❤️ halo*', id)
            } else if (args[1].toLowerCase() === 'disable') {
        ...
        case '❤️otakudesu':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️otakudesu [query]*\nContoh : *❤️otakudesu darling in the franxx*', id)
        ...
        case '❤️kusonime':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️kusonime [query]*\nContoh : *❤️kusonime darling in the franxx*', id)
        ...
        case '❤️dewabatch':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️dewabatch [query]*\nContoh : *❤️dewabatch darling in the franxx*', id)
        ...
        case '❤️komiku':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️komiku [query]*\nContoh : *❤️komiku darling in the franxx*', id)
        ...
        case '❤️pinterest':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️pinterest [query]*\nContoh : *❤️pinterest NEKO*', id)
        ...
        case '❤️nhview':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️nhview [212121]*\nContoh : *❤️nhview 321421*', id)
        ...
        case '❤️loli':
            ...
            tobz.sendFileFromUrl(from, loly.result, 'loli.jpeg', '*INI LOLI MU MZ SIAPIN KARUNK*', id)
            break
        case '❤️shota':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            tobz.sendFile(from, img, "shota.jpg", `*PENIKMAT SHOTA*`, id)
                    }) 
                .catch(
                    (error) => {
                        console.log(error);
        ...
        case '❤️nekonime':
            ...
            tobz.sendFileFromUrl(from, nekon.result, `Nekonime${ext}`, 'miauww', id)
            break
        case '❤️randomtrapnime':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            tobz.sendImage(from, trapn.result, `trapnime${ext}`, 'Turappu!', id)
            break
        case '❤️randomhentai':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            ...
            tobz.sendImage(from, henta.result, `RandomHentai${ext}`, 'oniichan Hentai!', id)
            break
        case '❤️randomnsfwneko':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            tobz.sendImage(from, nsfwn.result, `NsfwNeko${ext}`, 'SAMNGE', id)
            break
        case '❤️randomanime':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case '❤️nekopoi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️nekopoi [linkNekopoi]*\nContoh : *❤️nekopoi https://nekopoi.care/tsunpuri-episode-1-subtitle-indonesia/*', id)
            try {
        ...
        case '❤️malanime':
            ...
            await limitAdd(serial)
            const keyword = message.body.replace('❤️malanime', '')
            try {
...
🌐️ *URL*: ${url}`
        ...
        case '❤️malcharacter':
            ...
            await limitAdd(serial)
            const keywords = message.body.replace('❤️malcharacter', '')
            try {
            const data = await fetch(
...
🌐️ *URL*: ${url}`
        ...
        case '❤️jadwalsholat':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `[❗] Kirim perintah *❤️jadwalShalat [ Daerah ]*\ncontoh : *❤️jadwalShalat Tangerang*\nUntuk list daerah kirim perintah *❤️listDaerah*`)
        ...
        case `p`:
            ...
            break
        case `bot`:
            tobz.sendStickerfromUrl(from, 'https://i.ibb.co/KX0fctz/IMG-20201212-WA2063.png')
            tobz.sendPtt(from, './media/Open.mp3', id)
            break
        ...
        case `❤️cersex`:
            ...
            break
        case `❤️pesankosong`:
            tobz.reply(from, ` ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏ ͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏`, id)
            break
        case '❤️motivasi':
            ...
            break
	case '❤️howgay':
	    if (args.length === 1) return tobz.reply(from, `Untuk mengetahui seberapa gay seseorang gunakan ❤️howgay namanya\n\nContoh: ❤️howgay burhan`, id)
        ...
        case '❤️cerpen':
            ...
            break
        case '❤️cersex1':
            axios.get(`https://arugaz.herokuapp.com/api/cersex1`)
            .then((res) => {
                let hasil = `${res.result.data.article}`;
                tobz.reply(from, hasil, id)
            })
            break
        case '❤️cersex2':
            axios.get(`https://arugaz.herokuapp.com/api/cersex2`)
            .then((res) => {
                let hasil = `${res.result.data.article}`;
                tobz.reply(from, hasil, id)
            })
        ...
        case '❤️infosurah':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah *❤️infosurah [ Nama Surah ]*\nContoh : *❤️infosurah al-fatihah*`, message.id)
        ...
        case '❤️tafsir':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah *❤️tafsir [ Nama Surah ] [ Ayat ]*\nContoh : *❤️tafsir al-fatihah 2*`, message.id)
        ...
        case '❤️infogempa':
            ...
            await limitAdd(serial)
            const bmkg = await axios.get('https://mhankbarbars.herokuapp.com/api/infogempa'+'&apikey='+barbarkey)
        ...
        case '❤️ssweb':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️ssweb [linkWeb]*\nContoh : *❤️ssweb https://neonime.vip*', id)
        ...
        case '❤️shorturl':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️shorturl [linkWeb]*\nContoh : *❤️shorturl https://neonime.vip*', id)
        ...
        case '❤️cuaca':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️cuaca [tempat]*\nContoh : *❤️cuaca tangerang', id)
        ...
        case '❤️spamcall':
            ...
            console.log(spam)
            const call2 = await axios.get('https://mhankbarbars.herokuapp.com/api/spamcall?no=' + spam)
        ...
        case '❤️ytmp4':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️ytmp4 [ Link Yt ]*, untuk contoh silahkan kirim perintah *❤️readme*`)
        ...
        case '❤️play':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️ceklimit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return tobz.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: ❤️play judul lagu`, id)
            try {
                tobz.reply(from, mess.wait, id)
        ...
        case '❤️ytmp3':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️ytmp3 [ Link Yt ]*, untuk contoh silahkan kirim perintah *❤️readme*`, id)
        ...
        case '❤️xnxx':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️xnxx [linkXnxx]*, untuk contoh silahkan kirim perintah *❤️readme*')
        ...
        case '❤️ramalpasangan':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️ramalpasangan [kamu|pasangan]*\nContoh : *❤️ramalpasangan Tobz|NEKO*', id)
            arg = body.trim().split('|')
            if (arg.length >= 2) {
            tobz.reply(from, mess.wait, id)
            const kamu = arg[0]
        ...
        case '❤️artinama':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️artinama [query]*\nContoh : *❤️artinama Tobz*', id)
            try {
        ...
        case '❤️zodiak':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️zodiak [zodiak kamu]*\nContoh : *❤️zodiak scorpio*', id)
            try {
       ...
       case'❤️caklontong':
            ...
            tobz.reply(from, jwban, id)
                ...
                tobz.sendText(ownerNumber, 'caklontong : ' + err)
           }
           break
         case '❤️tebakgambar':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '❤️heroml':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️heroml [nama hero]*\nContoh : *❤️heroml akai*', id)
            try {
        ...
        case '❤️nomorhoki':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️nomorhoki [no hp kamu]*\nContoh : *❤️nomorhoki 0895384009405*', id)
            try {
        ...
        case '❤️artimimpi':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️artimimpi [mimpi]*\nContoh : *❤️artimimpi ular*', id)
            try {
         ...
         case '❤️fb':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️fb [ Link Fb ]*\nContoh : *❤️fb https://www.facebook.com/24609282673/posts/10158628585367674/*`, id)
            try {
            tobz.reply(from, mess.wait, id)
        ...
        case '❤️tiktok':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️tiktok [linkTiktok]*\nContoh : *❤️tiktok https://vt.tiktok.com/yqyjPX/*', id)
        ...
        case '❤️wiki':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️wiki [ Query ]*\nContoh : *❤️wiki asu*`, id)
        ...
        case '❤️kbbi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️kbbi [ Query ]*\nContoh : *❤️kbbi asu*`, id)
        ...
        case '❤️googleimg':
            ...
            const qwery = argz[1]
            const jum = argz[2]
            if(!qwery) return await tobz.reply(from, `Kirim perintah *❤️googleimage [ |Query|Jumlah ]*, contoh = ❤️googleimage |loli|3`, id)
            if(!jum) return await tobz.reply(from, `Jumlah gambar diperlukan, contoh = ❤️googleimage |loli|3`, id)
        ...
        case '❤️smule':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️smule [linkSmule]*\nContoh : *❤️smule https://www.smule.com/p/767512225_3062360163*', id)
            tobz.reply(from, mess.wait, id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
...
➸ *Type:* ${Type}`
        ...
        case '❤️sandwriting':
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️sandwriting [ Teks ]*\nContoh *❤️sandwriting NEKO Cantik*', id)
...
*Text : ${swrt}*`
         ...
         case '❤️tahta':
             ...
             if (!jreng) return tobz.reply(from, 'Kirim perintah *❤️tahta [teks]*\n\nContoh *❤️tahta NEKO*', id)
             if (jreng.length > 10) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
         ...
         case '❤️bpink':
             ...
             await limitAdd(serial)
             const bpink = body.slice(7)
             if (!bpink) return tobz.reply(from, 'Kirim perintah *❤️bpink [teks]*\n\nContoh *❤️bpink NEKO*', id)
             if (bpink.length > 10) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
             tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
             await tobz.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${bpink}&apikey=${vhtearkey}`,`${bpink}.jpg`,`noh ${bpink}`, id)        
             break
         case '❤️thundertext':
             if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
             if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
             ...
             await limitAdd(serial)
             const thtext = body.slice(13)
             if (!thtext) return tobz.reply(from, 'Kirim perintah *❤️thundertext [text]*\n\nContoh *❤️thundertext NEKOBOT*', id)
             tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
             await tobz.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${thtext}&apikey=${vhtearkey}`,`${phub}.jpg`,`noh`, id)        
             break
         case '❤phub':
             f (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
             if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
		     if (args.length == 2) return tobz.reply(from, `ketik ❤ph [teks 1] [teks 2]`, id)
		     await tobz.sendFileFromUrl(from, `https://=${args[1]}&text2=${args[2]}&apikey=api`, 'ph.jpg', '', id)
		     break
        case '❤️resep':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️resepmasakan [optional]*\nContoh *❤️resepmasakan rawon*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Cara:* ${cara}`
        ...
        case '❤️twtstalk':
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️twtstalk @username*\nContoh *❤️twtstalk @miakhalifah*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Jumlah Postingan:* ${status_count}`
        ...
        case '❤️igstalk':
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️igstalk @username*\nContoh *❤️igstalk duar_amjay*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Private:* ${is_private}`
        ...
        case '❤️tiktokstalk':
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️tiktokstalk @username*\nContoh *❤️tiktokstalk @duar_amjay*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Link:* ${url_account}`
        ...
        case '❤️smulestalk':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️smulestalk [@username]*\nContoh : *❤️smulestalk loli*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Total Rekaman:* ${recording}`
        ...
        case '❤️':
            ...
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️ [teks]*\nContoh : *❤️ halo*')
        ...
        case '❤️instagram':
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *❤️ig [ Link Instagram ]* untuk contoh silahkan kirim perintah *❤️readme*`)
        ...
        case '❤️starmaker':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️starmaker [linkStarmaker]* untuk contoh silahkan kirim perintah *❤️readme*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Deskripsi:* ${desc}`
        ...
        case '❤️maps':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️maps [optional]*, Contoh : *❤️maps Jakarta*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
        ...
        case '❤️twitter':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️twitter [linkTwitter]* untuk contoh silahkan kirim perintah *❤️readme*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Filesize:* ${filesize}`
        ...
        case '❤️joox':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️joox [optional]*\nContoh : *❤️joox Alan Walker*', id)
            tobz.reply(from, mess.wait, id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
...
➸ *Durasi:* ${duration}`
        ...
        case '❤️checkip':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️checkip [ipaddress]*\nContoh : *❤️checkip 182.0.144.145*', id)
            tobz.reply(from, mess.wait, id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
...
➸ *Wilayah:* +${region_name}`
        ...
        case '❤️nh':
            ...
            if (args.length === 2) {
                ...
                tobz.reply(from, '[ WRONG ] Kirim perintah *❤️nhentai [kode]* untuk contoh kirim perintah *❤️readme*')
            }
            break
        case '❤️brainly':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '❤️math':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length === 1) return tobz.reply(from, '[❗] Kirim perintah *❤️math [ Angka ]*\nContoh : ❤️math 12*12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /')
            const mtk = body.slice(6)
            if (typeof Math_js.evaluate(mtk) !== "number") {
            tobz.reply(from, `"${mtk}", bukan angka!\n[❗] Kirim perintah *❤️math [ Angka ]*\nContoh : ❤️math 12*12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /`, id)
        } else {
        ...
        case '❤️quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                tobz.reply(from, 'Usage: \n❤️quotemaker |teks|watermark|theme\n\nEx :\n❤️quotemaker |ini contoh|bicit|random', id)
            }
            break
        case '❤️listchannel':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '❤️jadwaltv':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️jadwalTv [channel]*', id)
        ...
        case '❤️nulis':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️nulis [teks]*, contoh *❤️nulis aku bukan boneka*', id)
        ...
        case '❤️quotes':
            ...
            await limitAdd(serial)
            const quotez2 = await axios.get('https://mhankbarbars.herokuapp.com/api/randomquotes')
        ...
        case '❤️lirik':
            ...
            await limitAdd(serial)
            if (args.length == 1) return tobz.reply(from, 'Kirim perintah *❤️lirik [optional]*, contoh *❤️lirik aku bukan boneka*', id)
        ...
        case '❤️chord':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *❤️chord [query]*, contoh *❤️chord aku bukan boneka*', id)
            const query__ = body.slice(7)
            const chord = await axios.get('https://mhankbarbars.herokuapp.com/api/chord?q='+ query__+'&apiKey='+ barbarkey)
            if (chord.data.error) return tobz.reply(from, chord.data.error, id)
            tobz.reply(from, chord.data.result, id)
            break
        case '❤️listdaerah':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            const listDaerah = await axios.get('https://mhankbarbars.herokuapp.com/daerah')
            tobz.reply(from, listDaerah.data.result, id)
            break
        // ADMIN & OWNER
        case '#bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner NEKO`, id)
                bctxt = body.slice(4)
                txtbc = `*「 NEKO BROADCAST 」*\n\n${bctxt}`
        ...
        case '❤️bc':
            ...
            const urlbc = 'https://www.linkpicture.com/q/IMG-20201212-WA1559.jpg'
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) 
                await tobz.sendFileFromUrl(ids, `${urlbc}`,'bc.jpg', `[ *NEKO BROADCAST* ]\n\n${msg}`)
            }
        ...
        case '❤️ownergroup':
            ...
            break
        case '❤️otagall': // FOR OWNER & ADMIN NEKO
        case '❤️omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner NEKO', id)
        ...
        case '❤️okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '❤️oadd':
            const orang = args[1]
            if (!isGroupMsg) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
            if (args.length === 1) return tobz.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *❤️add* 628xxxxx', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '❤️okick':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner NEKO', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return tobz.reply(from, 'Untuk menggunakan Perintah ini, kirim perintah *❤️okick* @tagmember', id)
        ...
        case '❤️kick':
            ...
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return tobz.reply(from, 'Untuk menggunakan Perintah ini, kirim perintah *❤️kick* @tagmember', id)
        ...
        case '❤️oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            await tobz.sendText(from,'NEKO DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '❤️leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '❤️opromote':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return tobz.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *❤️promote* @tagmember', id)
        ...
        case '❤️odemote':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return tobz.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *❤️demote* @tagadmin', id)
        ...
        case '❤️odelete':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
            if (!quotedMsg) return tobz.reply(from, 'Salah!!, kirim perintah *❤️delete [tagpesanbot]*', id)
        ...
        case '❤️getses':
            ...
            break
        case '❤️nekoadmin':
            let admn = `This is list of NEKO Admin\nTotal : ${adminNumber.length}\n`
        ...
        break
        case '❤️restart': // WORK IF YOU RUN USING PM2
            if(isOwner){
        ...
        case '❤️addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner NEKO!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin NEKO!', id)
                }
            break
        case '❤️deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner NEKO!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin NEKO!', id)
            break
        case '❤️block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner NEKO!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '❤️unblock':
            ...
            break
        case '❤️ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin NEKO!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin NEKO!`, id)
        ...
        case '❤️setgroupicon':
            ...
            if (isMedia) {
                ...
                tobz.reply(from, `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan ❤️setgroupicon`, id)
            }
            break
        case '❤️bugreport':
            if (args.length === 1) return tobz.reply(from, '[❗] Kirim perintah *❤️bugreport [teks]*\ncontoh : *❤️bugreport Permisi Owner, Ada bug pada command ❤️otakudesu, Tolong diperbaiki*')
        ...
        case '❤️help':
            ...
            break
        case '❤️nekogroup':
            tobz.reply(from, `Link Group NEKO : https://chat.whatsapp.com/DYmn7BJGVH0Am9qU5uZYgY\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '❤️groupmenu':
        ...
        case '❤️iklan':
            ...
            break
        case '❤️adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin NEKO', id)
        ...
        case '❤️snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup NEKO, Link Ada Di Bio atau Bisa Mengetik *❤️nekogroup*\nJika Ingin Sewa Bot Harap Ketik *❤️iklan*', id)
            if (command.startsWith('❤️')) {
                tobz.reply(from, `sudah kubilang ${pushname}, Command *${args[0]}* Tidak Terdaftar,makanya ketik *❤️menu*!`, id)
            }
