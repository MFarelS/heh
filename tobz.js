...
*Text : ${swrt}*`
         ...
         case '❤️thundertext':
             ...
             tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
             await tobz.sendFileFromUrl(from, `https://api.vhtear.com/thundertext?text=${thtext}&apikey=${vhtearkey}`,`${thtext}.jpg`,`noh`, id)        
             break
         case '❤️phub':
             if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
             if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
		     if (args.length == 2) return tobz.reply(from, `ketik ❤phub [teks 1] [teks 2]`, id)
		     await tobz.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${args[1]}&text2=${args[2]}&apikey=${vhtearkey}`, 'ph.jpg', '', id)
		     break
        case '❤️resep':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ❤️limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1)  return tobz.reply(from, 'Kirim perintah *❤️resep [optional]*\nContoh *❤️resep rawon*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
